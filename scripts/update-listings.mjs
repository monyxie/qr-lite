...
class AmoUpdater {
  ...
  async genToken() {
    console.log(
      "Get your key and secret at https://addons.mozilla.org/en-US/developers/addon/api/key/"
    );
    if (!this.key) {
  ...
  async getDetail() {
    const response = await fetch(
      `${this.getBaseUrl()}/addons/addon/${this.getExtensionId()}/`,
      {
        method: "GET",
      }
    );
  ...
  async updateMetadata() {
    ...
    // api docs: https://mozilla.github.io/addons-server/topics/api/addons.html#edit
    // build request body
    const body = { name: {}, summary: {}, description: {} };
    for (const locale of locales) {
      const messages = await readJSONFile(
        `./promo/_locales/${locale}/messages.json`
      );
      body.name[locale] = messages.extension_name.message;
  ...
  async updatePreviews() {
    ...
    if (
      ...
      (await confirm({
        message: `Delete the existing ${detail.previews.length} previews?`,
      }))
    ) {
  ...
  async start() {
    if (this.checkSiteStatus()) {
      if (await confirm({ message: "Update name, summary and description?" })) {
...
class MsEdgeUpdater {
  ...
  async getCredentials() {
    console.log(
      "Get your credentials at https://partner.microsoft.com/en-us/dashboard/microsoftedge/publishapi"
    );
    if (!this.clientId) {
  ...
  async start() {
    if (await confirm({ message: "Update title, summary and description?" })) {
      console.log(
        "hohoho, ms has not yet provided an api for updating product metadata yet"
      );
    }
  }
}
